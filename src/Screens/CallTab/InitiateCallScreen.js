...
import { getUserById, showToast, isCurrentRoute } from "../../utility/utils";
...
export default function VideoIncomingCallScreen({ route, navigation }) {
  ...
  console.log("[InitiateCallScreen]", callSession, isIcoming, currentUser);
  ...
  useEffect(() => {
    console.log("[isIcoming, isEarlyAccepted]", isIcoming, isEarlyAccepted);
  ...
  const startCall = async callType => {
    ...
    setTimeout(() => {
      navigation.push("VideoScreen", { initiatorName: currentUser?.full_name });
    }, 400);
  };
